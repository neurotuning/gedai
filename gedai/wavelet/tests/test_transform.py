...
def test_modwt_haar_single_band_shape_and_dtype():
    ...
    # float32 input
    data_f32 = rng.standard_normal((n_times, n_ch)).astype(np.float32)
    ...
    # float64 input
    data_f64 = rng.standard_normal((n_times, n_ch)).astype(np.float64)
...
def test_modwt_haar_single_band_numpy_torch_parity():
    ...
    data = rng.standard_normal((n_times, n_ch)).astype(np.float32)
    ...
    for level in [3, 5]:
        for band in range(level + 1):
            ...
            assert max_err < 1e-6, (
                f"Parity failure level={level}, band={band}, err={max_err}"
            )
...
def test_apply_wavelet_highpass_prefilter():
    ...
    data = (rng.standard_normal((n_ch, n_times)) * 0.1 + drift).astype(np.float32)
