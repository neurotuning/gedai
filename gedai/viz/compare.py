    ...
    def plot_window():
        ...
        if plot_mode == 'diff':
            for ch in range(n_chans):
                diff = (noisy_data[ch, start_idx:end_idx] - clean_data[ch, start_idx:end_idx]) * mag_scale
                ax.plot(times[start_idx:end_idx], diff + offsets[ch], color='purple', alpha=0.8, linewidth=0.8, label='Noisy - Cleaned' if ch == 0 else None)
            ...
            for ch in range(n_chans):
                ax.plot(times[start_idx:end_idx], clean_data[ch, start_idx:end_idx] * mag_scale + offsets[ch], color='blue', alpha=0.7, linewidth=0.8, label='Cleaned' if ch == 0 else None)
            ax.set_ylabel('Channels')
            ax.legend(loc='upper right')
            if title:
            ...
            for ch in range(n_chans):
                ax.plot(times[start_idx:end_idx], noisy_data[ch, start_idx:end_idx] * mag_scale + offsets[ch], color='red', alpha=0.7, linewidth=0.8, label='Noisy' if ch == 0 else None)
            ax.set_ylabel('Channels')
            ax.legend(loc='upper right')
            if title:
