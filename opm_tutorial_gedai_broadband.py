...
# 5. Apply GEDAI
# ==========================================
print(f"Applying Broadband GEDAI to {len(raw_mag.ch_names)} magnetometers...", flush=True)
gedai = Gedai(wavelet_level=0)
gedai.fit_raw(raw_mag, reference_cov=fwd, noise_multiplier=3.0, n_jobs=1)
raw_gedai = gedai.transform_raw(raw_mag, n_jobs=1)
...
# Get the first 17 or so channels out of the 85 to match the original visually
...
ax.grid(True)
ax.set(title="After Bandpass & Broadband GEDAI", ylim=(-500, 500), xlim=time_gedai[[0, -1]], xlabel="Time (s)", ylabel="Amplitude (pT)")
...
fig_erp = evoked.plot(picks="meg", window_title="GEDAI Evoked ERP", show=False)
...
fig_joint.suptitle("GEDAI Evoked Joint Map")
